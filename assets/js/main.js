...
const toggleOffcanvas = (offcanvasId) => {
  ...
  function handleOutsideClick(event) {
    if (
      ...
      !event.target.closest(`[data-toggle-offcanvas="${offcanvasId}"]`)
    ) {
...
const manageCollapse = () => {
  const toggleIcon = (icon, isCollapsed) => {
    icon?.classList.replace(
      isCollapsed ? "bi-chevron-down" : "bi-chevron-right",
  ...
  $$(".nav__item--folder").forEach((folder) => {
    const isCollapsed =
      sessionStorage.getItem(`collapsed_${folder.id}`) === "true";
...
const highlightActiveLink = () => {
  ...
  $$(".nav--tree .nav__link--file").forEach((link) => {
...
// TOC Highlighter
const highlightTOC = () => {
  const tocLinks = $$(".nav--toc .nav__link--toc");
  const headings = $$(
...
const initSearch = async () => {
  try {
    ...
    $("#search-input").addEventListener(
      "input",
      debounce(() => handleSearch(new Event("submit")))
    );
...
const manageNavbarWrapper = () => {
  $$(".navbar__wrapper").forEach((wrapper) => {
    ...
    const updateVisibility = () => {
      groups.forEach(
        (group, index) =>
          (group.style.display = index === currentIndex ? "block" : "none")
      );
